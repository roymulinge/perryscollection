...
class ProductAdmin(admin.ModelAdmin):
    ...
    def make_available(self, request, queryset):
        """
        Mark products available only when they satisfy
        the model's availability rules.

        We intentionally use product.save() instead of queryset.update()
        so Product.full_clean() runs.
        """
        ...
        if updated:
            self.message_user(
                request,
                f'{updated} product(s) marked as available.',
            )

        if skipped:
            self.message_user(
                request,
                f'{skipped} product(s) skipped because they have '
                f'zero stock or are inactive.',
                level='warning',
            )
    ...
    def make_unavailable(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} product(s) marked as unavailable.',
        )
    ...
    def add_stock_10(self, request, queryset):
        """
        Add 10 units to each selected product.

        Using save() ensures model validation runs.
        """
        ...
        self.message_user(
            request,
            f'Added 10 stock to {updated} product(s).',
        )
